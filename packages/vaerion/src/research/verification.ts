...
export async function verifyEvidence(evidence: EvidenceRecord, blobs: BlobStore): Promise<EvidenceVerificationItem> {
  ...
  // The excerpt is the FENCED view of the content: fenceUntrusted truncates
  // documents longer than the fence cap and marks the cut with an ellipsis.
  // The digest + fingerprint checks above already bind the FULL bytes
  // cryptographically; this check asserts record↔bytes agreement. A
  // truncated excerpt therefore verifies by its verbatim prefix.
  if (content.includes(evidence.excerpt)) {
    // full agreement
  } else if (evidence.excerpt.endsWith("…") && content.includes(evidence.excerpt.slice(0, -1))) {
    return {
      evidence_id: evidence.evidence_id,
      ok: true,
      detail: "blob + fingerprint + truncated fenced excerpt verified (content exceeds the fence cap; full bytes are digest-bound)",
    };
  } else {
