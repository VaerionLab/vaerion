...
import { aiStatusPayload, aiSetupPayload, aiUsePayload, aiRemovePayload, capabilityRows } from "./ai.ts";
import type { VaerionConfig } from "../config/config.ts";
import { createAccount, linkAccount, unlinkThisDevice, validateAccountImport, loadAccount, accountPath } from "./account.ts";
...
function ioLineSafe(ctx: CommandContext, line: string): void {
  ctx.io.out(line);
}
...
async function runWorkflow(ctx: CommandContext): Promise<number> {
  ...
  // Path law: --dag resolves against the WORKSPACE root (ctx.cwd), never the
  // ambient process cwd — the workspace is the only path anchor (D-M′).
  const raw = await rf(resolve(ctx.cwd, dagPath), "utf8").catch(() => {
...
export async function cmdDoctor(ctx: CommandContext): Promise<number> {
  const ws = workspaceAt(ctx.cwd);
  const checks: Array<{ check: string; ok: boolean; code?: string; detail?: string; impact?: string; fix?: string }> = [];
  ...
  if (cfgExists) {
    try {
      ...
      checks.push({ check: "config", ok: false, code: e.code, detail: e.message, impact: "an unreadable config blocks every governed surface — the engine refuses to guess", fix: e.fix });
    }
  } else {
  ...
  // 6. Blob orphans (maintenance, never a health failure): the SAME
  // conservative orphan law `vae clean` uses — any hash occurrence keeps it.
  {
    const blobsDir = ws.blobsDir;
    const hasBlobs = await stat(blobsDir).then(() => true, () => false);
    let orphans = 0;
    if (hasBlobs) {
      const onDisk: string[] = [];
      async function walkDir(dir: string): Promise<void> {
        for (const item of await readdir(dir, { withFileTypes: true })) {
          ...
          if (item.isDirectory()) await walkDir(full);
          else if (item.isFile()) onDisk.push(full);
        }
      }
      await walkDir(blobsDir);
      let refText = "";
      for (const f of (await readdir(ws.journalDir).catch(() => [] as string[])).filter((f) => f.endsWith(".ndjson"))) {
        refText += await readFile(join(ws.journalDir, f), "utf8").catch(() => "");
      }
      for (const extra of [ws.auditPath, ws.refusalsPath]) {
        refText += await readFile(extra, "utf8").catch(() => "");
      }
      orphans = onDisk.filter((p) => !refText.includes(basename(p))).length;
    }
    checks.push({
      check: "blob-orphans",
      ok: true, // maintenance intelligence, not a health failure — the audit stays honest either way
      detail: orphans === 0 ? "no orphaned blobs" : `${orphans} orphaned blob(s) — \`vae clean --yes\` reclaims them`,
      fix: orphans === 0 ? undefined : "run `vae clean --yes`",
    });
  }
...
export async function cmdAccount(ctx: CommandContext): Promise<number> {
  const sub = ctx.flags._positional1;
  if (sub === "create") {
    const name = reqFlag(ctx, "name");
    const email = typeof ctx.flags.email === "string" && ctx.flags.email.length > 0 ? String(ctx.flags.email) : null;
    const { account, device, created } = await createAccount(name, email);
    r(ctx).result({
      command: "account",
      kind: "create",
      created,
      ...
      note: "local-first identity — the account file lives on this device; there is no cloud account (XVIII-3)",
    });
    return ExitCode.ok;
  }
  if (sub === "login") {
    const file = reqFlag(ctx, "file");
    const raw: unknown = JSON.parse(await readFile(file, "utf8"));
    const imported = validateAccountImport(raw);
    if (!imported) throw new VaerionError("E1600", "the file is not a valid Vaerion account export. Fix: export from the device that holds the account, then re-run `vae account login --file PATH`");
    const { account, device } = await linkAccount(imported);
    r(ctx).result({ command: "account", kind: "login", account_id: account.account.id, name: account.account.name, device_id: device.id, note: "this device is now linked to the imported account" });
    return ExitCode.ok;
  }
  if (sub === "logout") {
    const res = await unlinkThisDevice();
    r(ctx).result({ command: "account", kind: "logout", ...res, note: "the local identity file is untouched — re-link with `vae account login --file PATH`" });
    return ExitCode.ok;
  }
  if (sub === "export") {
    const account = await loadAccount();
    if (!account) throw new VaerionError("E1600", "no account on this device. Fix: run `vae account create --name NAME`");
    r(ctx).result({ command: "account", kind: "export", account, note: "this JSON is the account export — keep it; it links other devices via `vae account login --file PATH`" });
    return ExitCode.ok;
  }
  if (sub !== "" && sub !== "status") {
    throw new VaerionError("E1600", "usage: `vae account` | `vae account status` | `vae account create --name NAME [--email E]` | `vae account login --file PATH` | `vae account logout` | `vae account export`");
  }
  const ws = workspaceAt(ctx.cwd);
...
export async function cmdAi(ctx: CommandContext): Promise<number> {
  const sub = ctx.flags._positional1;
  if (sub !== "ask" && sub !== "models" && sub !== "status" && sub !== "setup" && sub !== "use" && sub !== "remove") {
    throw new VaerionError("E1600", "usage: `vae ai ask --question TEXT [--sources P,P | --capability NAME] [--model P/M] [--seed N]` | `vae ai models` | `vae ai status` | `vae ai setup --provider P --model M [--set-default] [--dry-run]` | `vae ai use --model M` | `vae ai remove --provider P`");
  }
  ...
  if (sub === "status" || sub === "setup" || sub === "use" || sub === "remove") {
    const ws = workspaceAt(ctx.cwd);
    if (sub === "status") {
      if (ctx.flags._positional2 !== "") {
        throw new VaerionError("E1600", "usage: `vae ai status` takes no arguments — it reports the developer-owned AI layer");
      }
      const { config } = await loadOrAdhocConfig(ws);
      const payload = await aiStatusPayload(config);
      r(ctx).result({ command: "ai", kind: "status", engine_version: ENGINE_VERSION, ...payload });
      return ExitCode.ok;
    }
    if (sub === "setup") {
      const provider = reqFlag(ctx, "provider");
      const model = reqFlag(ctx, "model");
      const payload = await aiSetupPayload({
        wsRoot: ctx.cwd,
        provider,
        model,
        setDefault: ctx.flags["set-default"] === true,
        dryRun: ctx.flags["dry-run"] === true || ctx.dryRun === true,
      });
      r(ctx).result({ command: "ai", kind: "setup", ...payload });
      return ExitCode.ok;
    }
    if (sub === "use") {
      const model = reqFlag(ctx, "model");
      const payload = await aiUsePayload({ wsRoot: ctx.cwd, model, dryRun: ctx.flags["dry-run"] === true || ctx.dryRun === true });
      r(ctx).result({ command: "ai", kind: "use", ...payload });
      return ExitCode.ok;
    }
    const provider = reqFlag(ctx, "provider");
    const payload = await aiRemovePayload({ wsRoot: ctx.cwd, provider, dryRun: ctx.flags["dry-run"] === true || ctx.dryRun === true });
    r(ctx).result({ command: "ai", kind: "remove", ...payload });
    return ExitCode.ok;
  }
...
/* ────────────  status / report / clean — the dashboard trio (Phase 16)  ──────────── */

/** `vae status` — the project dashboard (catalogued, read-only): workspace
 *  state + identity, the gateway default model, run totals, and the next
 *  hints. The editor status bar consumes exactly this payload. */
export async function cmdStatus(ctx: CommandContext): Promise<number> {
  ...
  const state = hasConfig || hasWorkspace ? "workspace" : "fresh";
  let name = "adhoc";
  let fingerprint: string | null = null;
  let defaultModel: string | null = null;
  let configForAi: VaerionConfig | undefined;
  if (hasConfig) {
    const { config, fingerprint: fp } = await loadOrAdhocConfig(ws);
    name = config.project?.name ?? "adhoc";
    fingerprint = fp;
    defaultModel = config.gateway?.defaultModel ?? null;
    configForAi = config;
  }
  // Identity (XVIII-3): local-first, presence only — values never leave the device.
  const account = await loadAccount();
  const accountPayload: Record<string, unknown> = account
    ? { present: true, account_id: account.account.id, name: account.account.name }
    : { present: false };
  // The ONE fold (measureCenter) feeds runs + recent — never a second implementation.
  let repoRoot: string | null = null;
  try {
    repoRoot = (await discoverRepository(ctx.cwd)).root;
  } catch {
  ...
  const center = await measureCenter({
    root: ws.root,
    journalDir: ws.journalDir,
    blobsDir: ws.blobsDir,
    auditPath: ws.auditPath,
  ...
  const total = center.workspace.runs;
  const closed = center.operations.runs.filter((r) => r.state === "closed").length;
  const recent = [...center.operations.runs]
    .sort((a, b) => (a.run_id < b.run_id ? 1 : -1))
    .slice(0, 5)
    .map((r) => ({ run_id: r.run_id, kind: r.kind, state: r.state, closed_at: r.closed_at, records: r.records }));
  const aiRows = await capabilityRows(configForAi ?? ({ gateway: {} } as VaerionConfig));
  const hints = state === "fresh"
    ? ["vae init — scaffold the governed workspace", "vae tour — the guided walk of the engine, measured against this machine"]
    : [
        `vae doctor — audit this workspace: config, journals, blobs, evidence, audit chain`,
        `vae center — the operator cockpit: runs, receipts, metering, integrity`,
        ...(center.operations.runs.some((r) => r.state === "open") ? ["an open run is waiting — `vae resume RUN_ID` continues it"] : []),
      ];
  const payload = {
    command: "status",
    engine_version: ENGINE_VERSION,
    workspace: { state, name, ...(fingerprint ? { config_fingerprint: fingerprint } : {}) },
    account: accountPayload,
    ai: {
      default_model: defaultModel,
      credentials: aiRows.map((r) => ({ provider: r.provider, credential: r.credential })),
    },
    runs: { total, closed, open: total - closed },
    recent,
    hints,
...
/** `vae report` — a pure fold over every journal (catalogued, read-only):
 *  run totals, gateway metering, and the journaled evidence-record count.
 *  The editor tree view renders the highlights. */
export async function cmdReport(ctx: CommandContext): Promise<number> {
  const ws = workspaceAt(ctx.cwd);
  let repoRoot: string | null = null;
  try {
    repoRoot = (await discoverRepository(ctx.cwd)).root;
  ...
  const metering = report.operations.metering;
  const byModel: Record<string, Record<string, number>> = {};
  for (const [model, m] of Object.entries(metering.byModel)) {
    byModel[model] = { invocations: m.invocations, failed: m.failed, input_tokens: m.inputTokens, output_tokens: m.outputTokens, total_micro_usd: m.totalMicroUsd };
  }
  const openRun = report.operations.runs.find((r) => r.state === "open");
  const reads: string[] = [];
  if (report.workspace.runs === 0) reads.push("no runs yet — the journals are empty; `vae run demo` writes the first receipt");
  if (openRun !== undefined) reads.push(`an open run exists — \`vae resume ${openRun.run_id}\` continues it`);
  if (report.evidenceRecorded > 0) reads.push("evidence is recorded — `vae doctor` verifies the chains");
  if (report.operations.journals_verified) reads.push("every journal chain verifies — replay is deterministic");
  const payload = {
    command: "report",
    engine_version: ENGINE_VERSION,
    runs: { total: report.workspace.runs, by_kind: report.byKind },
    metering: {
      invocations: metering.invocations,
      failed: metering.failed,
      input_tokens: metering.inputTokens,
    ...
    evidence: { recorded: report.evidenceRecorded },
    span: report.span,
    journals_verified: report.operations.journals_verified,
    reads,
    note: "a pure fold over every journal — insight, not vanity; the same journals always yield the same numbers",
    read_only: "a pure fold over the journals — nothing was created or modified",
  };
  r(ctx).result(payload);
  return ExitCode.ok;
}

/** `vae clean` — reclaim orphaned blobs (catalogued): plan-only by default,
 *  --yes applies, --dry-run is a plan that never applies. The orphan law is
 *  the conservative one: a blob is orphaned only when its hash appears in NO
 *  journal, NO audit entry, and NO refusal record — evidence-linked blobs
 *  are never candidates. */
export async function cmdClean(ctx: CommandContext): Promise<number> {
  const ws = workspaceAt(ctx.cwd);
  const hasWorkspace = await stat(ws.vaerionDir).then(() => true, () => false);
  if (!hasWorkspace) {
    // A non-workspace is a teaching payload, not an error (the clean law).
    r(ctx).result({
      command: "clean",
      engine_version: ENGINE_VERSION,
      scanned: 0,
      ...
      orphan_bytes: 0,
      deleted: false,
      note: "not a Vaerion workspace — nothing to clean. Fix: run `vae init`",
    });
    return ExitCode.ok;
  }
  ...
  await walk(ws.blobsDir);
  // The reference scan is raw-text on purpose: ANY occurrence of the hash
  // (a journal event, an audit entry, a refusal record) keeps the blob.
  let refText = "";
  for (const f of (await readdir(ws.journalDir).catch(() => [] as string[])).filter((f) => f.endsWith(".ndjson"))) {
    refText += await readFile(join(ws.journalDir, f), "utf8").catch(() => "");
  }
  ...
  const orphans = onDisk.filter((p) => !refText.includes(basename(p)));
  let totalOrphanBytes = 0;
  for (const p of orphans) totalOrphanBytes += (await stat(p)).size;
  const apply = yes && !dryRun;
  let reclaimed = 0;
  if (apply) {
    for (const p of orphans) {
      reclaimed += (await stat(p)).size;
      await rm(p);
    }
  }
  const note = apply
    ? `reclaimed ${orphans.length} orphaned blob(s) (${reclaimed} bytes) — evidence-linked blobs were never candidates`
    : dryRun
      ? `dry-run — nothing deleted (--yes would reclaim ${orphans.length} orphan(s))`
      : `plan-only by default — ${orphans.length} orphan(s) found; re-run with --yes to reclaim`;
  const payload = {
    command: "clean",
    engine_version: ENGINE_VERSION,
    scanned: onDisk.length,
    referenced: onDisk.length - orphans.length,
    orphan_count: orphans.length,
    orphan_bytes: totalOrphanBytes,
    orphans: orphans.map((p) => relative(ws.root, p).split("\\").join("/")).slice(0, 100),
    deleted: apply,
    reclaimed_bytes: reclaimed,
    dry_run: dryRun,
    note,
    law: "a blob is orphaned only when its hash appears in no journal, audit, or refusal record",
  };
  if (ctx.mode === "json") r(ctx).result(payload);
  else for (const line of [`scanned: ${onDisk.length} blob(s), ${orphans.length} orphaned`, note]) ioLineSafe(ctx, line);
  return ExitCode.ok;
}
...
export async function buildWelcomePayload(ctx: CommandContext): Promise<Record<string, unknown>> {
  ...
  const nextSuggestions = kind === "fresh"
    ? [
        "vae init — scaffold vaerion.yaml + .vaerion/ in this directory — nothing exists yet",
    ...
    : [
        "vae doctor — audit this workspace: config, journals, blobs, evidence, audit chain, gateway matrix",
        "vae status — the project dashboard: workspace, identity, AI, runs, next steps",
        "vae center — the operator cockpit: runs, receipts, metering, integrity",
      ];
  return {
    command: "welcome",
...
export async function cmdTour(ctx: CommandContext): Promise<number> {
  ...
  const steps = [
    ...
    },
    {
      ...
      try: "vae package build · vae provenance <ARTIFACT>",
      note: "Trust is measured, never assumed (D-S): git identity and canonical protection (D-P/D-Q), CI as the remote projection of the single verification authority (D-R), and a fail-closed release evaluator. Reproducible bundles and permanent provenance close the loop.",
    },
    {
      step: 9,
  ...
  // The designed machine shape (editor + docs contract): n/title/detail/command
  // per step — measured and try ride along for the rich renderer.
  const typedSteps = steps.map((s) => ({
    n: s.step,
    step: s.step,
    title: s.title,
    detail: s.note,
...
// Recovery surface (Phase 16): snapshot/restore implementations live in
// their own module (src/cli/recovery.ts — the module's intended purpose);
// re-exported here so the dispatch imports stay single-sourced.
export { cmdSnapshot, cmdRestore } from "./recovery.ts";
