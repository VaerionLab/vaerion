...
- Launch preparation (Phase 12 — Launch Readiness & Externalization Order):
  version alignment of the surfaces outside the engine register (root
  `package.json`, `editors/vscode`, `editors/jetbrains`,
  `BETA-ONBOARDING.md`, `SECURITY.md`, `SUPPORT.md`, `examples/README.md`
  — the editor manifests are now INSIDE the version register, pinned by
  `version-register.test.ts`); `editors/README.md` (was an empty file);
  `.github/CODEOWNERS`; `.env.example` (names only — no secret values);
  `tools/launch/credential-check.ts` (`bun run launch:credential-check`);
  `docs/operations/LAUNCH-SEQUENCE.md`; the launch website layer
  (`src/components/launch/`, displayed at the host route pending IR-021);
  `brand/OFFICIAL-LOGO-SLOT.md` (the official-logo reservation — no logo
  was created or altered); `VAERION_LAUNCH_READINESS_REPORT.md`. No
  engine code, constitutional law, or ratified value changed.
