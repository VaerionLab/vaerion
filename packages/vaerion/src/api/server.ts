...
/** The wire contract is enforced, not decorative: a route's declared
 *  requestBody is the law — property types, required keys, and
 *  additionalProperties:false are all checked here (D-V: the fix removes the
 *  defect class; silently-coerced bodies can never betray the schema). */
function assertBodyAgainstSchema(body: Record<string, unknown>, schema: Record<string, unknown>): void {
  const required = Array.isArray(schema.required) ? (schema.required as string[]) : [];
  for (const key of required) {
    if (body[key] === undefined) throw new VaerionError("E1600", `body.${key} is required`);
  }
  const properties = (schema.properties ?? {}) as Record<string, { type?: string }>;
  for (const [key, value] of Object.entries(body)) {
    const prop = properties[key];
    if (!prop) {
      throw new VaerionError("E1600", `unknown body property: ${key}`);
    }
    const t = prop.type;
    if (t === "string" && typeof value !== "string") throw new VaerionError("E1600", `body.${key} must be a string`);
    if (t === "boolean" && typeof value !== "boolean") throw new VaerionError("E1600", `body.${key} must be a boolean`);
    if (t === "number" && typeof value !== "number") throw new VaerionError("E1600", `body.${key} must be a number`);
  }
}

async function readJsonBody(request: Request, route?: { requestBody?: Record<string, unknown> }): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    if (route?.requestBody !== undefined) assertBodyAgainstSchema(body, route.requestBody);
    return body;
  } catch (err) {
...
async function handle(request: Request, route: DaemonRoute, params: Record<string, string>, ctx: HandlerContext): Promise<Response> {
  ...
  switch (route.operationId) {
    ...
    case "startRun": {
      const body = await readJsonBody(request, route);
