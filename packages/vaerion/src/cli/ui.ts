 ...
 * assertions. Tokens mirror the official palette of record
 * (brand/official/MANIFEST.md — the Founder upload is the only identity;
 * PHASE 16.3 purged brand/BRAND-BOOK.md, brand/tokens.json and every
 * non-official identity file — the former generator tool/brand-render.ts
 * was removed in PHASE 16.1).
 */
/**
...
/**
 * The terminal identity banner — PHASE 16.2 ABSOLUTE BRAND LAW.
 * The former drawn mark (witness rule + chevron V) is REMOVED: no drawn
 * glyph may stand in for the official identity, and a terminal cannot
 * render the official raster. The banner is now typographic only — the
 * wordmark, version, and tagline. It is not a logo; it is text.
 */
export function banner(a: Ansi, version: string, width: number): string[] {
  ...
  for (const lines of right) {
    out.push(" ".repeat(offset) + lines.join(" "));
  }
  return out;
}
...
export function renderRichResult(a: Ansi, obj: Obj, width: number): string[] {
  ...
  switch (command) {
    ...
    case "status": return statusReport(a, obj, width);
...
function statusReport(a: Ansi, obj: Obj, width: number): string[] {
  const out: string[] = [];
  const wsObj = sub(obj, "workspace") ?? {};
  const account = sub(obj, "account") ?? {};
  const ai = sub(obj, "ai") ?? {};
  const runs = sub(obj, "runs") ?? {};
  const recent = Array.isArray(obj.recent) ? (obj.recent as Array<Record<string, unknown>>) : [];
  const hints = Array.isArray(obj.hints) ? (obj.hints as string[]) : [];
  out.push(...panel(a, {
    title: "Workspace",
    lines: kvBlock(a, [
      ["state", str(wsObj, "state") ?? "?"],
      ["name", str(wsObj, "name") ?? "?"],
      ...(wsObj.config_fingerprint !== undefined ? [["fingerprint", str(wsObj, "config_fingerprint") ?? "?"] as [string, string]] : []),
    ], width),
    width,
    accent: str(wsObj, "state") === "fresh" ? "info" : "gold",
  }));
  out.push("");
  out.push(...panel(a, {
    title: "Identity",
    lines: account.present === true
      ? kvBlock(a, [["account id", str(account, "account_id") ?? "?"], ["name", str(account, "name") ?? "?"]], width)
      : [a.dim("no local account — `vae account create --name NAME` creates one (optional by law)")],
    width,
    accent: "info",
  }));
  out.push("");
  const creds = Array.isArray(ai.credentials) ? (ai.credentials as Array<{ provider?: string; credential?: string }>) : [];
  out.push(...panel(a, {
    title: "AI",
    lines: [
      `default: ${str(ai, "default_model") ?? "(none)"}`,
      ...creds.map((c) => `${a.dim(SYM.dot)} ${c.provider}: credential ${c.credential}`),
    ],
    width,
    accent: "info",
  }));
  out.push("");
  out.push(...panel(a, {
    title: `Runs — ${str(runs, "total") ?? "0"} (${str(runs, "closed") ?? "0"} closed / ${str(runs, "open") ?? "0"} open)`,
    lines: recent.length > 0
      ? recent.map((r) => `${a.dim(SYM.dot)} ${str(r, "run_id") ?? "?"} · ${str(r, "kind") ?? "?"} · ${str(r, "state") ?? "?"}`)
      : [a.dim("no runs journaled in this workspace yet")],
    width,
    accent: "gold",
  }));
  out.push("");
  out.push(...panel(a, {
    title: "Next",
    lines: hints.map((h) => `${a.gold("▸")} ${h}`),
    width,
    accent: "success",
  }));
  out.push("");
  out.push(a.dim(str(obj, "read_only") ?? ""));
  return out;
}
...
function welcomeReport(a: Ansi, obj: Obj, width: number): string[] {
  ...
  const nextLines = Array.isArray(obj.next)
    ? (obj.next as string[]).map((suggestion) => `${a.gold("▸")} ${suggestion}`)
    : (() => {
        const next = sub(obj, "next") ?? {};
        return [`${a.gold("▸")} ${str(next, "command") ?? "vae --help"} — ${str(next, "why") ?? ""}`];
      })();
  const learn = Array.isArray(obj.learn) ? (obj.learn as string[]) : [];
  out.push("");
  out.push(...panel(a, {
    title: "Your environment",
    subtitle: kind === "fresh" ? "Start here — this directory is fresh, nothing is governed yet" : "Start here — this is a Vaerion workspace",
    lines: kvBlock(a, [
      ["path", str(dir, "path") ?? "?"],
...
interface TourStepShape { step?: number; n?: number; title?: string; measured?: string[]; try?: string; command?: string; note?: string }
...
function tourReport(a: Ansi, obj: Obj, width: number): string[] {
  ...
  out.push(...panel(a, {
    ...
    width,
    accent: "gold",
    subtitle: "read-only by law · no network · no writes · measured, never assumed (D-S)",
  }));
  for (const s of steps) {
    ...
    lines.push(s.note ?? "");
    const tryCmd = (s.command !== undefined && s.command.length > 0) ? s.command : (s.try ?? "");
    if (tryCmd.length > 0) lines.push(a.gold(`$ ${tryCmd}`));
    out.push(...panel(a, {
