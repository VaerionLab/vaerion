 ...
 * registry — it is deliberately OUT of this register (kept aligned by hand
 * for surface coherence only). The engine, npm-consumer, SDK, and tools
 * packages, every packaging manifest, the editor manifests, and the
...
const REGISTER: Array<[string, string[]]> = [
  ...
  // editor ecosystem manifests (Phase 12 launch preparation — the editor
  // surfaces must carry the engine version of record like every other
  // distributable)
  ["editors/vscode/package.json", [`"version": "${VERSION}"`]],
  ["editors/jetbrains/build.gradle.kts", [`version = "${VERSION}"`]],
  // python channel
