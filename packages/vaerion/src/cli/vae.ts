...
import { buildWelcomePayload, cmdAccount, cmdAi, cmdCenter, cmdClean, cmdDev, cmdExplain, cmdInit, cmdJournal, cmdDoctor, cmdPackage, cmdProvenance, cmdRepo, cmdRelease, cmdRestore, cmdResume, cmdRun, cmdServe, cmdCi, cmdSnapshot, cmdStatus, cmdReport, cmdTour, type CommandContext } from "./commands.ts";
...
Command surface (the Daily Seven + additive commands):
  init [--template T] [--name NAME] [--dry-run]
                             scaffold vaerion.yaml + .vaerion/ from the
                             deterministic template registry (templates:
                             minimal|demo|agent; bare init is exactly
                             --template minimal)
  status                      the project dashboard: workspace, identity,
                             AI, runs, next steps (read-only)
  report                      a pure fold over every journal — runs,
                             decisions, metering, evidence (read-only)
  clean [--yes] [--dry-run]   reclaim orphaned blobs — plan-only by default;
                             evidence-linked blobs never
  ...
  snapshot [--out FILE] [--dry-run]
                             deterministic archive of the evidence state
                             (whitelist: config, journals, audit chain,
                             blob CAS) pinned by its blake3 digest —
                             identical state, identical bytes; --dry-run
                             plans without writing (the plan IS the
                             outcome)
  restore FILE [--force] [--dry-run]
                             verify-then-restore an archive; conflicts
                             refuse with the list until --force;
                             journals re-verified after the bytes land
  ...
  account create --name NAME [--email E]
                             local-first identity: create, status, login,
                             logout, export (optional by law). Read-only
                             measures: who acts in this workspace (XVIII-3),
                             the actors observed in the journals, the
                             commit identity (D-P), and declared secret
                             PROFILES — names only. Local identity,
                             never a cloud account.
  ai setup --provider P --model M [--set-default] [--dry-run]
                             declare a provider ceiling in vaerion.yaml
                             (and optionally the default selection)
  ...
  ai status                  the developer-owned AI layer: providers,
                             credential PRESENCE (never values), defaults
...
vae ci simulate --event push|pull_request|workflow_dispatch|tag [--ref NAME]
  ...
  them. Read-only by law: the tour creates nothing, journals nothing, and
  touches nothing beyond stat(). The same directory yields byte-identical
  --json output.`,
  account: `vae account
...
export async function runCli(argv: string[], io: CliIo, cwd: string): Promise<CliResult> {
  ...
  try {
    ...
    switch (parsed.command) {
      ...
      case "status": code = await cmdStatus(ctx); break;
      case "report": code = await cmdReport(ctx); break;
      case "clean": code = await cmdClean(ctx); break;
      ...
      case "snapshot": code = await cmdSnapshot(ctx); break;
      case "restore": code = await cmdRestore(ctx); break;
      case "help": {
        ...
        err.code === "E1600" || err.code === "E1203" || err.code === "E1700" || err.code === "E1701" || err.code === "E2204" || err.code === "E2300"
          ...
          : err.code === "E1300" || err.code === "E1301" || err.code === "E1302"
            ...
            : err.code === "E1702" || err.code === "E1704" || err.code === "E1705" || err.code === "E1706" || err.code === "E1601"
              ...
              : err.code === "E1703" || err.code === "E2200" || err.code === "E2201" || err.code === "E2202" || err.code === "E2203" || err.code === "E2205" || err.code === "E2206" || err.code === "E1501"
                ? ExitCode.partial
