# VAERION CONSTITUTION — v1.2

| | |
|---|---|
| **Document** | VAERION_CONSTITUTION_v1.2 — the ratified law of the Vaerion engine |
...
| **Supersedes** | v1.1 (retained unmodified at `VAERION_CONSTITUTION_v1.1.md` as historical record); v1.0 (retained unmodified at `VAERION_CONSTITUTION_v1.0.md`) |
...
through D-O) and the ASCENSION XVIII amendment session (D-M′, D-P through D-T; see
...
| D-M′ | Command surface law *(supersedes D-M "Daily Seven")* | The CLI command surface grows only by ratified Founder directive, and every command honors the Five Guarantees (D-N). The surface of record is `vae --help` — help and dispatch never disagree. The ratified surface at v1.2: `init, run, resume, explain, journal, doctor, dev, serve, package, provenance, repo, ci, release, tour`. The bare invocation (`vae` with no command) is the **welcome front door**: a read-only teaching surface that measures this directory and points at the next step — it exits 0 in every output mode and is never a usage error. (Supersession rationale in §11-A1; surface growth in §11-A2.) |
...
- **What changed.** D-M′ surface list: `tour` added (14 commands). D-M′ now also
