...
| A5 tampers with release artifacts | Reproducible build (ADR-0016): identical inputs → byte-identical `.vxn` (blake3 identity, pinned zstd-19); release distribution publishes sha256 + blake3 manifests and an Ed25519 signature over the manifest (`tools/dist-verify.ts` is the consumer-side check) | T (P2 proof: two builds byte-identical) + release tooling |
