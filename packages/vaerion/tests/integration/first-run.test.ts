...
describe("bare `vae` — the welcome front door (constitution v1.2 D-M′/A2)", () => {
  ...
  test("--json is a single pure line with the stable contract", async () => {
    ...
    // Phase 16 contract resolution: `next` is the designed array of adaptive
    // suggestions (cli-first-run.test.ts pins the same shape; status.hints
    // carries it too) — the single-suggestion object was the earlier draft.
    const next = payload["next"] as string[];
    expect(next[0]?.startsWith("vae init")).toBe(true);
  ...
  test("detects an existing workspace and points at `vae doctor`", async () => {
    ...
    const next = payload["next"] as string[];
    expect(next[0]?.startsWith("vae doctor")).toBe(true);
  });
...
describe("D-M′ — help and dispatch never disagree (v1.6 surface)", () => {
