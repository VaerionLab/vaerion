...
describe("the remote protection wiring", () => {
  test("the slug is parsed from the measured remote URL, never hardcoded", () => {
    expect(slugFromRemoteUrl("https://auren-via-credential-file@github.com/VaerionLab/vaerion.git")).toBe(
      "VaerionLab/vaerion",
    );
    expect(slugFromRemoteUrl("git@github.com:VaerionLab/vaerion.git")).toBe("VaerionLab/vaerion");
