 ...
 */
/** The one lifecycle-count fold (single source — receipt and `vae report` both use it). */
...
export function lifecycleCounts(records: readonly JournalRecord[]): LifecycleCounts {
...
export function openedAtOf(records: readonly JournalRecord[]): string | null {
  for (const rec of records) {
    if (rec.k === "meta" && rec.note === "header" && typeof rec.opened_at === "string") return rec.opened_at;
  }
  return null;
}
...
export function buildReceiptFromRecords(records: JournalRecord[], opts: { closedAt: string; engineVersion: string; summary: string }): RunReceipt {
  const counts = lifecycleCounts(records);
  let runId = "";
  let traceId = "";
  ...
  const openedAt = openedAtOf(records);
